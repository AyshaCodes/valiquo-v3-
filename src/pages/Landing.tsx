...
function DemoScanner({ navigate }: { navigate: (to: Page) => void }) {
  ...
  // Score intelligent basé sur des mots-clés marocains réalistes
  const getMockScore = (text: string) => {
    const lower = text.toLowerCase();
    if (lower.includes('livraison') || lower.includes('livreur')) return 78;
    if (lower.includes('coiffeur') || lower.includes('beauté')) return 65;
    if (lower.includes('plomberie') || lower.includes('réparation')) return 72;
    if (lower.includes('application') || lower.includes('app')) return 68;
    if (lower.includes('stock') || lower.includes('inventaire')) return 74;
    if (lower.includes('éducation') || lower.includes('formation')) return 70;
  ...
  const handleDemo = () => {
    ...
    setTimeout(() => {
      setAnalyzing(false);
      const score = getMockScore(problem);
...
export default function Landing({ navigate }: LandingProps) {
  ...
  const steps = [
    { num: '01', titre: 'Décris le problème', desc: 'Exprime la frustration que tu observes (ex: "les livreurs perdent du temps à cause des adresses imprécises").' },
    { num: '02', titre: 'Lance le scan IA', desc: 'Notre IA scrute les réseaux sociaux, forums et médias marocains pour trouver des preuves de demande réelle.' },
    { num: '03', titre: 'Reçois ton rapport', desc: 'Score de validation (0-100), analyse SWOT, contexte marché et plan d’action priorisé.' },
    { num: '04', titre: 'Construis en confiance', desc: 'Utilise le Coach IA pour affiner ta stratégie et passe à l’action avec des étapes concrètes.' },
  ];
    ...
    <div className="relative z-10">
      <section className="min-h-screen flex flex-col items-center justify-center pt-20 pb-16">
        <div className="container text-center">
          ...
          >
            Valiquo analyse ton idée de startup sur le marché marocain via l’IA. Reçois un score de validation, une analyse SWOT et un plan d’action concret – avant d’investir un seul dirham.
          </p>
        ...
        <div className="container">
          <div className="card p-8 md:p-10 max-w-3xl mx-auto">
            ...
            <div className="space-y-3">
              {[
                { role: 'assistant', msg: 'Ton scan montre une demande solide (78/100) pour une solution de livraison à Casablanca. J’ai identifié 3 segments prioritaires. Lequel veux-tu explorer ?' },
                { role: 'user', msg: 'Les livreurs indépendants.' },
                { role: 'assistant', msg: 'Excellent. Environ 12 000 livreurs actifs à Casablanca. Je te propose un guide d’entretien terrain pour valider leur besoin réel cette semaine. Veux-tu le recevoir ?' },
              ].map((m, i) => (
                <div
                  key={i}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {plans.map((plan, i) => (
              ...
              >
                ...
                {plan.badge && (
                  <div className="text-xs mt-1" style={{ color: 'var(--accent)' }}>{plan.badge}</div>
                )}
                <p className="text-xs mb-5 mt-2 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{plan.desc}</p>
