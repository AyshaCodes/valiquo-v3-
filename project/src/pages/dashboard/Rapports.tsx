...
const reports = [
  { id: 1, project: 'Livraison de repas healthy', score: 81, date: '08 Juin 2026', status: 'Prêt' },
  { id: 2, project: 'Marketplace artisans marocains', score: 68, date: '03 Juin 2026', status: 'Prêt' },
  { id: 3, project: 'Location de plantes d\'intérieur', score: 55, date: '27 Mai 2026', status: 'En cours' },
];
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      {hasReports ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800 text-left">
                <th className="pb-3 text-slate-500 font-medium text-xs">Projet</th>
                <th className="pb-3 text-slate-500 font-medium text-xs">Score</th>
            ...
            <tbody>
              {reports.map((r) => (
                <tr key={r.id} className="border-b border-slate-800/50 hover:bg-slate-900/40 transition">
                  <td className="py-3 text-white">{r.project}</td>
                  <td className="py-3">
                    <span className={`font-syne font-700 ${r.score >= 70 ? 'text-emerald-400' : r.score >= 50 ? 'text-amber' : 'text-red-400'}`}>{r.score}/100</span>
                  </td>
        ...
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-slate-500 text-sm text-center max-w-xs">Lance un scan pour générer ton premier rapport d'analyse détaillé.</p>
        </div>
      )}
    </div>
  );
