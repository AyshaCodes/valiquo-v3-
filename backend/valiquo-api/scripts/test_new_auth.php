...
echo "=== TESTING NEW AUTH ENDPOINTS ===\n\n";

$testEmail = 'testnewauth' . time() . '@example.com';
...
// Step 2: Register with new endpoint
echo "Step 2: POST /api/auth/register\n";
echo "Email: {$testEmail}\n";
try {
    $response = Http::withOptions(['verify' => false])
        ->asJson()
        ->post('http://localhost:8000/api/auth/register', [
            'name' => 'Test New Auth User',
            'email' => $testEmail,
...
// Step 3: Login with new endpoint
echo "Step 3: POST /api/auth/login\n";
try {
    $response = Http::withOptions(['verify' => false])
        ->asJson()
